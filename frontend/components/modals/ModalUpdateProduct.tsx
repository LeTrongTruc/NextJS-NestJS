...
const App: React.FC<props> = ({ isOpenModalLogin, setIsOpenModalLogin, id }) => {
    ...
    const callApiDetailProduct = async () => {
        if (id) {
            const data = await POST_ADMIN('/product/detailProduct', { id });
            if (data.result) {
                return data.data
            }
        }
    ...
    const queryDetail = useQuery({ queryKey: ['detailProduct'], queryFn: callApiDetailProduct });
    const [image, setImage] = useState();

    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        const response = await POST_ADMIN_FORM('/product/updateProduct', {
    ...
    useEffect(() => {
        queryClient.invalidateQueries({ queryKey: ['detailProduct'] })
        ...
        <>
            <Modal footer={null} open={isOpenModalLogin} onOk={() => setIsOpenModalLogin(false)} onCancel={() => setIsOpenModalLogin(false)}>
                <form onSubmit={handleSubmit(onSubmit)} className='w-full flex justify-center flex-col items-center'>
                    <div className='text-xl font-medium'>Update Product</div>
                    <div className='w-full'>
                        ...
                        <select className='w-full h-10 border outline-none '{...register('category_id')}
                            defaultValue={queryDetail?.data?.category_id}
                        ...
                        <input
                            ...
                            defaultValue={queryDetail?.data?.detail}
                        ...
                        <div className='flex justify-between'>
                            <div className='w-[49%]'>
                                ...
                                <input
                                    ...
                                    defaultValue={queryDetail?.data?.specification}
                            ...
                            <div className='w-[49%]'>
                                ...
                                <input
                                    ...
                                    className="bg-gray-50 border border-gray-300 outline-none text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white mt-3 dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    {...register('standard')}
                                    defaultValue={queryDetail?.data?.standard}
                        ...
                        <div className='flex justify-between'>
                            <div className='w-[49%]'>
                                ...
                                <input
                                    ...
                                    className="bg-gray-50 border border-gray-300 outline-none text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white mt-3 dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    {...register('quantity')}
                                    defaultValue={queryDetail?.data?.quantity}
                            ...
                            <div className='w-[49%]'>
                                ...
                                <input
                                    ...
                                    className="bg-gray-50 border border-gray-300 outline-none text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white mt-3 dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    {...register('unit')}
                                    defaultValue={queryDetail?.data?.unit}
                        ...
                        <input defaultValue={queryDetail?.data?.images} className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 p-3" id="file_input" type="file" onChange={handleChangeFile} />
