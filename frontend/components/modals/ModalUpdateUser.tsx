...
const App: React.FC<props> = ({
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    const response = await POST_ADMIN_FORM("/admin/updateInfo", {
      ...data,
      _id: id,
      file: image,
  ...
  const callApiDetailUser = async () => {
    if (id) {
      const data = await POST_ADMIN("/admin/detailUser", { id });
      if (data.result) {
        return data.data;
      }
    }
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="text-2xl font-semibold">Update info user</div>
