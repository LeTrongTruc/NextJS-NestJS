...
const App: React.FC = () => {
  ...
  const columns: TableColumnsType = [
    ...
    },
    {
      ...
      dataIndex: "role",
      className: "bg-white text-center border",
      render: () => <div className="text-base">Customer</div>,
    },
    {
      title: "Status",
      className: "border",
  ...
  const requestListUser = async () => {
    const data = await POST_ADMIN("/admin/listUser", {
  ...
  const query = useQuery({ queryKey: ["listUser"], queryFn: requestListUser });
  ...
  const handleDelete = async () => {
    if (selectedRowKeys.length > 0) {
      const response = await POST_ADMIN("/admin/deleteUser", {
        _id: selectedRowKeys,
      });
      if (response.result) {
  ...
  return (
    <div className="w-[98%] mr-5 mt-2">
      ...
      <div className="text-2xl font-semibold">List user</div>
